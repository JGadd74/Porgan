"""This module is responsible for logging and dry run functions."""
import os
import logging
import zipfile
...
class FileIOReporter:
    ...
    def __init__(self, move_mode, archive_mode, remove_duplicates_mode, log_level=logging.INFO, data_fetcher=None):
        # CLI args...
    ...
    def dry_move(self, file_dict):
        """This method simulates moving files to their respective folders"""
    ...
    def dry_archive(self, file_dict):
        """This method simulates archiving files to their respective archives"""
        # TODO test
            ...
            for file in file_list:
                ...
                if os.path.exists(f'{file}'):
                    ...
                    # check if file is already present in archive
                        with zipfile.ZipFile(f'{target_directory}/{file_category}.zip', 'r') as zipf:
    ...
    def dry_remove_duplicates(self, file_list):
        """This method simulates removing duplicate files"""
        
        #TODO remove strings from file_list as if they were remove
        #TODO rename strings from file_list as if they were real files

        prefix = "Dry run: "
        duplicates, orphaned_duplicates = self.fetcher.get_duplicate_files(self.fetcher.get_file_list())#type: ignore
        ...
        if self.fetcher.settings['rename_orphaned_duplicates'] is True:#type: ignore

            self.logger.info(f'{prefix}Renaming {len(orphaned_duplicates)} orphaned duplicate(s)...')
        ...
        self.logger.info(f'{prefix}Removing {len(duplicates)} duplicate file(s)...')
        for file in duplicates:
            if os.path.isfile(file):
                self.logger.info(f'\tRemoving {os.path.basename(file)}...')
                files_removed += 1

        self.logger.info(f'Dry run complete. {files_removed} file(s) would be removed.\n')

    # orchestrate dry run
    def dry_run(self):
        """This method orchestrates a dry run of the file organizer"""
        ...
        mutable_file_list = list(self.fetcher.get_file_list())#type: ignore
        
        if self._dry_remove_duplicates:
            self.dry_remove_duplicates(mutable_file_list)
